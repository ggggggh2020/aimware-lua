...
xpcall(function()
local ffi = assert(ffi, "ffi not enabled")
...
local k32 = nil
local ok, err = pcall(function() k32 = ffi.load("kernel32") end)
if not ok or not k32 then
    ok, k32 = pcall(function() return ffi.C end)
    if not ok then
        print("[皮肤] 无法加载 kernel32，皮肤应用将不可用")
    end
end

local client_base = ffi.cast("uintptr_t", mem.GetModuleBase("client.dll"))

local CES = ffi.cast("uintptr_t*", client_base + 0x24D4E80)[0]

local function GetEntityInstance(idx)
    if idx <= 0 or idx > 0x7fff then return nil end
    local chunk = bit.rshift(idx, 9)
    local slot  = bit.band(idx, 0x1FF)
...
local SIG_REGEN = "48 83 EC ?? E8 ?? ?? ?? ?? 48 85 C0 0F 84 ?? ?? ?? ?? 48 8B 10"
local regen_addr = mem.FindPattern("client.dll", SIG_REGEN)
local fnRegen = regen_addr and ffi.cast("void(__fastcall*)()", regen_addr) or nil
print(string.format("[皮肤] Regenerate 函数地址: 0x%X", tonumber(regen_addr or 0)))

local SIG_MGM = "48 89 5C 24 ?? 48 89 74 24 ?? 57 48 83 EC ?? 48 8D 99 ?? ?? ?? ?? 48 8B 71"
local mgm_addr = mem.FindPattern("client.dll", SIG_MGM)
local fnSetMeshGroupMask = mgm_addr and
    ffi.cast("void(__fastcall*)(void*, uint64_t)", mgm_addr) or nil
print(string.format("[皮肤] SetMeshGroupMask 函数地址: 0x%X", tonumber(mgm_addr or 0)))

local SIG_SETMODEL = "40 53 48 83 EC 20 48 8B D9 4C 8B C2 48 8B 0D ?? ?? ?? ?? 48 8D 54 24"
local setmodel_addr = mem.FindPattern("client.dll", SIG_SETMODEL)
local fnSetModel = setmodel_addr and
    ffi.cast("void*(__thiscall*)(void*, const char*)", setmodel_addr) or nil
print(string.format("[皮肤] SetModel 函数地址: 0x%X", tonumber(setmodel_addr or 0)))

local SIG_UPDATE_SUB = "4C 8B DC 53 48 81 EC ?? ?? ?? ?? 48 8B 41"
local update_sub_addr = mem.FindPattern("client.dll", SIG_UPDATE_SUB)
local fnUpdateSubclass = update_sub_addr and
    ffi.cast("void(__thiscall*)(void*)", update_sub_addr) or nil
print(string.format("[皮肤] UpdateSubclass 函数地址: 0x%X", tonumber(update_sub_addr or 0)))
...
local EXPECTED_IMM = OFF.ATTR_MGR + OFF.ITEM + OFF.ATTR_LIST + OFF.ATTR_VEC

local ORIGINAL_IMM = 0x0AA8
print(string.format("[皮肤] EXPECTED_IMM = 0x%X (动态计算), ORIGINAL_IMM = 0x%X",
    EXPECTED_IMM, ORIGINAL_IMM))

local function u16(a) local o,v=pcall(function() return ffi.cast("uint16_t*",a)[0] end); return o and v or nil end
local function u64(a) local o,v=pcall(function() return ffi.cast("uint64_t*",a)[0] end); return o and v or nil end
local function q  (a) local o,v=pcall(function() return ffi.cast("uintptr_t*",a)[0] end); return o and v or nil end

local function protect_write(addr, size, value16)
    if not k32 then return false, "k32 未加载" end
    local old = ffi.new("uint32_t[1]")
    local PAGE_EXECUTE_READWRITE = 0x40

    local ok1, r1 = pcall(function()
        return k32.VirtualProtect(ffi.cast("void*", addr), size, PAGE_EXECUTE_READWRITE, old)
    end)
    if not ok1 or r1 == 0 then
        return false, "VirtualProtect 失败: " .. tostring(r1)
    end

    local old_prot = old[0]
    local ok2, err2 = pcall(function()
        ffi.cast("uint16_t*", addr)[0] = value16
    end)

    pcall(function()
        k32.VirtualProtect(ffi.cast("void*", addr), size, old_prot, old)
    end)

    if not ok2 then return false, "写入失败: " .. tostring(err2) end
    return true, string.format("old_prot=0x%X", old_prot)
end

_G._AK47_buf = ffi.new("uint8_t[?]", 0x48 * 3)

local function apply_ak_attributes(wpn, paint, seed, wear, legacy)
    local item_addr = wpn + OFF.ATTR_MGR + OFF.ITEM
    ffi.cast("int32_t*", item_addr + OFF.ITEM_ID_HIGH)[0] = -1
    ffi.cast("int32_t*", wpn + OFF.FB_PK)[0]   = paint
    ffi.cast("int32_t*", wpn + OFF.FB_SEED)[0] = seed
    ffi.cast("float*",   wpn + OFF.FB_WEAR)[0] = wear

    local buf = _G._AK47_buf
    ffi.fill(buf, 0x48 * 3, 0)
    ffi.cast("uint16_t*", buf + 0 * 0x48 + 0x30)[0] = 6
    ffi.cast("float*",    buf + 0 * 0x48 + 0x34)[0] = paint
    ffi.cast("float*",    buf + 0 * 0x48 + 0x38)[0] = paint
    ffi.cast("uint16_t*", buf + 1 * 0x48 + 0x30)[0] = 7
    ffi.cast("float*",    buf + 1 * 0x48 + 0x34)[0] = seed
    ffi.cast("float*",    buf + 1 * 0x48 + 0x38)[0] = seed
    ffi.cast("uint16_t*", buf + 2 * 0x48 + 0x30)[0] = 8
    ffi.cast("float*",    buf + 2 * 0x48 + 0x34)[0] = wear
    ffi.cast("float*",    buf + 2 * 0x48 + 0x38)[0] = wear

    local attr_list_addr = item_addr + OFF.ATTR_LIST + OFF.ATTR_VEC
    ffi.cast("uint64_t*",  attr_list_addr)[0]     = 3
    ffi.cast("uintptr_t*", attr_list_addr + 8)[0] = ffi.cast("uintptr_t", buf)

    local mask = legacy and 2 or 1
    local sn = q(wpn + OFF.GSN)
    if sn then

        if fnSetMeshGroupMask then
            pcall(fnSetMeshGroupMask, ffi.cast("void*", sn), mask)
        else

            ffi.cast("uint64_t*", sn + OFF.MODEL_STATE + OFF.MESH_MASK)[0] = mask
        end
    end

    local hud_list = entities.FindByClass("C_CS2HudModelWeapon")
    if hud_list then
        for _, e in ipairs(hud_list) do
            local hud_ptr = GetEntityInstance(e:GetIndex())
            if hud_ptr and hud_ptr ~= 0 then
                local hud_sn = q(hud_ptr + OFF.GSN)
                if hud_sn then
                    if fnSetMeshGroupMask then
                        pcall(fnSetMeshGroupMask, ffi.cast("void*", hud_sn), mask)
                    else
                        pcall(function()
                            ffi.cast("uint64_t*", hud_sn + OFF.MODEL_STATE + OFF.MESH_MASK)[0] = mask
                        end)
                    end
                end
            end
    ...
    return attr_list_addr
end

local function cleanup_attributes(wpn, attr_addr)
    ffi.cast("uint64_t*",  attr_addr)[0]     = 0
    ffi.cast("uintptr_t*", attr_addr + 8)[0] = 0
    ffi.cast("int32_t*", wpn + OFF.FB_PK)[0] = -1
end

local function get_active_weapon()
    local pawn = entities.GetLocalPawn()
    if not pawn then return nil end
    local pawn_ptr = GetEntityInstance(pawn:GetIndex())
    if not pawn_ptr or pawn_ptr == 0 then return nil end

    local ok1, ws = pcall(function() return ffi.cast("uintptr_t*", pawn_ptr + 0x11E0)[0] end)
    if not ok1 or not ws or ws == 0 then return nil end

    local ok2, handle = pcall(function() return ffi.cast("uint32_t*", ws + 0x60)[0] end)
    if not ok2 or not handle or handle == 0 or handle == 0xFFFFFFFF then return nil end

    local idx = bit.band(handle, 0x7FFF)
    local wpn = GetEntityInstance(idx)
    if not wpn or wpn == 0 then return nil end
    return wpn
end
...
[
    ...
    },
    {
        ...
        "paint_name": "格洛克 18 型 | Mirror Mosaic",
        "legacy_model": false
    },
    {
        "weapon_defindex": 4,
    ...
    },
    {
        ...
        "weapon_name": "weapon_knife_skeleton",
        "paint": "410",
        "image": "https://raw.githubusercontent.com/Nereziel/cs2-WeaponPaints/main/website/img/skins/weapon_knife_skeleton-410.png",
        "paint_name": "骷髅匕首（★） | 大马士革钢",
        "legacy_model": true
    },
    {
        "weapon_defindex": 525,
    ...
    },
    {
        ...
        "weapon_name": "weapon_knife_skeleton",
        "paint": "735",
        "image": "https://raw.githubusercontent.com/Nereziel/cs2-WeaponPaints/main/website/img/skins/weapon_knife_skeleton-735.png",
        "paint_name": "骷髅匕首（★） | 夜色",
        "legacy_model": false
    },
    {
...
local function parse_skin_json(json_str)
    local skins = {}

    json_str = json_str:gsub("^%s*%[", ""):gsub("%]%s*$", "")

    local pattern = "({[^{}]*})"
    for obj in json_str:gmatch(pattern) do

        local defindex = obj:match('"weapon_defindex":%s*(%d+)')
        if not defindex then goto continue end
        defindex = tonumber(defindex)

        if defindex >= 5030 then
            goto continue
        end
        local weapon_name = obj:match('"weapon_name":%s*"([^"]+)"')
        local paint = obj:match('"paint":%s*"?(%d+)"?')
        if paint then paint = tonumber(paint) end
        local paint_name = obj:match('"paint_name":%s*"([^"]+)"')
        local legacy_str = obj:match('"legacy_model":%s*(%a+)')
        local legacy = (legacy_str == "true")
        if paint and paint_name and paint ~= 0 then
            table.insert(skins, {
    ...
    table.sort(skins, function(a,b) return a.paint_name < b.paint_name end)
    return skins
end

local all_skins = parse_skin_json(skin_json_str)
print(string.format("[皮肤] 加载了 %d 个皮肤条目", #all_skins))

local KNIFE_DEF = {
    [500] = { sub = 0xEA728847, vmdl = "weapons/models/knife/knife_bayonet/weapon_knife_bayonet.vmdl" },
    [503] = { sub = 0xE1BCA0B3, vmdl = "weapons/models/knife/knife_css/weapon_knife_css.vmdl" },
    [505] = { sub = 0xF12F03A4, vmdl = "weapons/models/knife/knife_flip/weapon_knife_flip.vmdl" },
    [506] = { sub = 0x7A0D7E2A, vmdl = "weapons/models/knife/knife_gut/weapon_knife_gut.vmdl" },
    [507] = { sub = 0x6733320E, vmdl = "weapons/models/knife/knife_karambit/weapon_knife_karambit.vmdl" },
    [508] = { sub = 0x61AA7734, vmdl = "weapons/models/knife/knife_m9/weapon_knife_m9.vmdl" },
    [509] = { sub = 0x880BE10C, vmdl = "weapons/models/knife/knife_tactical/weapon_knife_tactical.vmdl" },
    [512] = { sub = 0xCB62F193, vmdl = "weapons/models/knife/knife_falchion/weapon_knife_falchion.vmdl" },
    [514] = { sub = 0x95B26E63, vmdl = "weapons/models/knife/knife_bowie/weapon_knife_bowie.vmdl" },
    [515] = { sub = 0x50AFF643, vmdl = "weapons/models/knife/knife_butterfly/weapon_knife_butterfly.vmdl" },
    [516] = { sub = 0xFE815574, vmdl = "weapons/models/knife/knife_push/weapon_knife_push.vmdl" },
    [517] = { sub = 0x41E8E89D, vmdl = "weapons/models/knife/knife_cord/weapon_knife_cord.vmdl" },
    [518] = { sub = 0x1072DC40, vmdl = "weapons/models/knife/knife_canis/weapon_knife_canis.vmdl" },
    [519] = { sub = 0x4FC9FC2F, vmdl = "weapons/models/knife/knife_ursus/weapon_knife_ursus.vmdl" },
    [520] = { sub = 0xC08CB159, vmdl = "weapons/models/knife/knife_navaja/weapon_knife_navaja.vmdl" },
    [521] = { sub = 0xBF22131D, vmdl = "weapons/models/knife/knife_outdoor/weapon_knife_outdoor.vmdl" },
    [522] = { sub = 0x9AB0CBD0, vmdl = "weapons/models/knife/knife_stiletto/weapon_knife_stiletto.vmdl" },
    [523] = { sub = 0xF0348BE1, vmdl = "weapons/models/knife/knife_talon/weapon_knife_talon.vmdl" },
    [525] = { sub = 0x15C1E578, vmdl = "weapons/models/knife/knife_skeleton/weapon_knife_skeleton.vmdl" },
    [526] = { sub = 0xE5326A34, vmdl = "weapons/models/knife/knife_kukri/weapon_knife_kukri.vmdl" },
}

local function is_knife_defindex(def)
    return def and KNIFE_DEF[def] ~= nil
end

local function ent_is_knife(e)
    if not e then return false end
    local name = e:GetModelName()
    return name and (name:find("knife") or name:find("bayonet"))
end

local OFF_HUD_OWNER_ENTITY = 0x520
local function find_my_knives()
    local out = {}
    local pawn = entities.GetLocalPawn()
    if not pawn then return out end
    local pawn_idx = pawn:GetIndex()
    if not pawn_idx then return out end

    local ok, weapons = pcall(entities.FindByClass, "C_CSWeaponBase")
    if not ok or not weapons then return out end
    for _, w in ipairs(weapons) do
        if ent_is_knife(w) then
            local wp = GetEntityInstance(w:GetIndex())
            if wp and wp ~= 0 then

                local ok_h, h = pcall(function()
                    return ffi.cast("uint32_t*", wp + OFF_HUD_OWNER_ENTITY)[0]
                end)
                if ok_h and h then
                    local owner_idx = bit.band(tonumber(h), 0x7FFF)
                    if owner_idx == pawn_idx then
                        out[#out+1] = { wpn = wp, ent = w }
                    end
                end
            end
        end
...
local function forge_econ_identity(wpn_ptr, target_defindex)
    if not wpn_ptr or wpn_ptr == 0 then return false end
    local item_addr = wpn_ptr + OFF.ATTR_MGR + OFF.ITEM
    pcall(function()

        ffi.cast("uint16_t*", item_addr + 0x1BA)[0] = target_defindex

        ffi.cast("uint32_t*", item_addr + OFF.ITEM_ID_HIGH)[0] = 0x7FFFFFFF
        ffi.cast("uint32_t*", item_addr + OFF.ITEM_ID_LOW)[0]  = 0x7FFFFFFF

        ffi.cast("uint32_t*", item_addr + OFF.ACCOUNT_ID)[0]   = 12345

        ffi.cast("uint8_t*",  item_addr + OFF.INITIALIZED)[0]  = 1
        ffi.cast("uint8_t*",  item_addr + OFF.DISALLOW_SOC)[0] = 0
    end)
    return true
end

local _model_cstr_cache = {}
local function cstr_path(path)
    if not _model_cstr_cache[path] then
        _model_cstr_cache[path] = ffi.new("char[?]", #path + 1, path)
    end
    return _model_cstr_cache[path]
end

local function switch_knife_type(target_defindex)
    local kd = KNIFE_DEF[target_defindex]
    if not kd then return false, "not a knife defindex" end

    local knives = find_my_knives()
    if #knives == 0 then return false, "no local knife entity found" end

    for _, k in ipairs(knives) do
        forge_econ_identity(k.wpn, target_defindex)
        pcall(function()
            ffi.cast("uint32_t*", k.wpn + OFF.SUBCLASS_ID)[0] = kd.sub
        end)
        if fnUpdateSubclass then
            pcall(fnUpdateSubclass, ffi.cast("void*", k.wpn))
        end
    end

    if fnSetModel and kd.vmdl and kd.vmdl ~= "" then
        local cs = cstr_path(kd.vmdl)
        for _, k in ipairs(knives) do
            if k.ent:GetModelName() ~= kd.vmdl then
                pcall(fnSetModel, ffi.cast("void*", k.wpn), cs)
            end
        end

        local ok_h, hud_list = pcall(entities.FindByClass, "C_CS2HudModelWeapon")
        if ok_h and hud_list then
            for _, e in ipairs(hud_list) do
                if ent_is_knife(e) and e:GetModelName() ~= kd.vmdl then
                    local inst = GetEntityInstance(e:GetIndex())
                    if inst and inst ~= 0 then
                        pcall(fnSetModel, ffi.cast("void*", inst), cs)
                    end
                end
            end
        end
    ...
    return true, string.format("switched %d knife entity(ies)", #knives)
end

local function reapply_single_knife(wpn_ptr, defindex, config)
    if not wpn_ptr or not is_knife_defindex(defindex) then return false end
    local kd = KNIFE_DEF[defindex]
    if not kd then return false end

    forge_econ_identity(wpn_ptr, defindex)

    pcall(function()
        ffi.cast("uint32_t*", wpn_ptr + OFF.SUBCLASS_ID)[0] = kd.sub
    end)
    if fnUpdateSubclass then
        pcall(fnUpdateSubclass, ffi.cast("void*", wpn_ptr))
    end

    if fnSetModel and kd.vmdl and kd.vmdl ~= "" then
        local cs = cstr_path(kd.vmdl)
        pcall(fnSetModel, ffi.cast("void*", wpn_ptr), cs)
    ...
    if config.paint and config.paint > 0 and regen_addr and fnRegen and k32 then
        local attr_addr
        local ok_attr = pcall(function()
            attr_addr = apply_ak_attributes(wpn_ptr, config.paint, config.seed, config.wear, config.legacy)
        end)
        if ok_attr then
            local ok_patch = protect_write(regen_addr + 0x52, 2, EXPECTED_IMM)
            if ok_patch then
                pcall(fnRegen)
                protect_write(regen_addr + 0x52, 2, ORIGINAL_IMM)
            end
            pcall(cleanup_attributes, wpn_ptr, attr_addr)
        end
    end
    return true
end

local function apply_paint_to_all_knives(paint, seed, wear, legacy)
    if not regen_addr or not fnRegen or not k32 then return false end
    if not paint or paint <= 0 then return true end
    ...
    for _, k in ipairs(knives) do
        ...
        local attr_addr
        local ok_attr = pcall(function()
            attr_addr = apply_ak_attributes(wp, paint, seed, wear, legacy)
        end)
        if ok_attr then
            local ok_patch = protect_write(regen_addr + 0x52, 2, EXPECTED_IMM)
            if ok_patch then
            ...
            pcall(cleanup_attributes, wp, attr_addr)
        end
    end
    return applied > 0
end
...
local weapon_list = {}
local weapon_to_skins = {}
local defindex_to_wname = {}
local seen_wpn = {}
for _, skin in ipairs(all_skins) do
    if not weapon_to_skins[skin.weapon_name] then
        weapon_to_skins[skin.weapon_name] = {}
    end
    table.insert(weapon_to_skins[skin.weapon_name], skin)
    defindex_to_wname[skin.defindex] = skin.weapon_name
    if not seen_wpn[skin.weapon_name] then
        seen_wpn[skin.weapon_name] = true
        table.insert(weapon_list, skin.weapon_name)
    end
end
table.sort(weapon_list)

local CONFIG_FILE = "auto_skin_config.txt"
local saved_configs = {}

local function save_skin_config(defindex, paint, seed, wear, legacy)

    local is_saving_knife = is_knife_defindex(defindex)

    local lines = {}
    local ok_read, content = pcall(file.Read, CONFIG_FILE)
    if ok_read and content and content ~= "" then
        for line in content:gmatch("[^\r\n]+") do
            local d = line:match("^(%d+)=")
            if d then
                local dn = tonumber(d)
                local drop = (dn == defindex)

                if is_saving_knife and is_knife_defindex(dn) then
                    drop = true
                end
                if not drop then
    ...
    table.insert(lines, string.format("%d=%d,%d,%.4f,%s", defindex, paint, seed, wear, tostring(legacy)))
    table.sort(lines)
    file.Write(CONFIG_FILE, table.concat(lines, "\n"))

    if is_saving_knife then
        for k in pairs(saved_configs) do
            if is_knife_defindex(k) and k ~= defindex then
                saved_configs[k] = nil
            end
        end
    end
    saved_configs[defindex] = {paint = paint, seed = seed, wear = wear, legacy = legacy}
    local wname = defindex_to_wname[defindex] or ("#" .. defindex)
    print(string.format("[皮肤] 配置已保存到 %s （%s）", CONFIG_FILE, wname))
end

local function load_skin_configs()
    saved_configs = {}
    local ok_read, content = pcall(file.Read, CONFIG_FILE)
    if ok_read and content and content ~= "" then
        for line in content:gmatch("[^\r\n]+") do
            local dstr, rest = line:match("^(%d+)=(.+)$")
            if dstr and rest then
                local parts = {}
                for p in rest:gmatch("[^,]+") do
                    table.insert(parts, p)
                end
                if #parts >= 4 then
                    local didx = tonumber(dstr)
                    saved_configs[didx] = {
                        paint = tonumber(parts[1]),
                        seed = tonumber(parts[2]),
                        wear = tonumber(parts[3]),
                        legacy = parts[4] == "true"
                    }
                end
            end
    ...
    local knife_defs = {}
    for k in pairs(saved_configs) do
        if is_knife_defindex(k) then knife_defs[#knife_defs+1] = k end
    end
    if #knife_defs > 1 then
        table.sort(knife_defs)
        local keep = knife_defs[#knife_defs]
        for _, d in ipairs(knife_defs) do
            if d ~= keep then saved_configs[d] = nil end
        end
        print(string.format("[皮肤] 清理重复刀配置：保留 def=%d，清除 %d 个",
            keep, #knife_defs - 1))

        local lines = {}
        for d, c in pairs(saved_configs) do
            table.insert(lines, string.format("%d=%d,%d,%.4f,%s",
                d, c.paint, c.seed, c.wear, tostring(c.legacy)))
        end
        table.sort(lines)
        pcall(file.Write, CONFIG_FILE, table.concat(lines, "\n"))
    end

    print(string.format("[皮肤] 已加载 %d 个自动应用配置", #saved_configs))
end

local function get_active_weapon_defindex()
    local pawn = entities.GetLocalPawn()
    if not pawn then return nil, nil end
    local pawn_ptr = GetEntityInstance(pawn:GetIndex())
    if not pawn_ptr or pawn_ptr == 0 then return nil, nil end
    local ok1, ws = pcall(function() return ffi.cast("uintptr_t*", pawn_ptr + 0x11E0)[0] end)
    if not ok1 or not ws or ws == 0 then return nil, nil end
    local ok2, handle = pcall(function() return ffi.cast("uint32_t*", ws + 0x60)[0] end)
    if not ok2 or not handle or handle == 0 or handle == 0xFFFFFFFF then return nil, nil end
    local idx = bit.band(handle, 0x7FFF)
    local ent = entities.GetByIndex(idx)
    if not ent then return nil, idx end
    return ent:GetWeaponID(), idx
end

local function apply_auto_skin(defindex, wpn_ptr, config)
    if not regen_addr or not fnRegen then return false end
    if not k32 then return false end

    if is_knife_defindex(defindex) then
        switch_knife_type(defindex)
        if config.paint and config.paint > 0 then
            apply_paint_to_all_knives(config.paint, config.seed, config.wear, config.legacy)
        end
        return true
    end

    if not wpn_ptr then return false end
    local attr_addr
    local ok_attr = pcall(function() attr_addr = apply_ak_attributes(wpn_ptr, config.paint, config.seed, config.wear, config.legacy) end)
    if not ok_attr then return false end
    local ok_patch, _ = protect_write(regen_addr + 0x52, 2, EXPECTED_IMM)
    if not ok_patch then
        pcall(cleanup_attributes, wpn_ptr, attr_addr)
        return false
    end
    local ok_regen = pcall(fnRegen)
    protect_write(regen_addr + 0x52, 2, ORIGINAL_IMM)
    cleanup_attributes(wpn_ptr, attr_addr)
    return ok_regen
end
...
local display_map = {
    ["weapon_bayonet"] = "KNIFE_BAYONET",
    ["weapon_m4a1"] = "M4A4",
    ["weapon_m4a1_silencer"] = "M4A1",
}

local wpn_display_names = {}
for _, w in ipairs(weapon_list) do

    local display = display_map[w] or w:gsub("^weapon_", ""):upper()
    table.insert(wpn_display_names, display)
end

local win = gui.Window("universal_skin", "通用皮肤应用 (VirtualProtect)", 100, 100, 700, 600)


local skin_box  = gui.Groupbox(win, "皮肤设置",  15,  20, 300, 1000)

local glove_box = gui.Groupbox(win, "手套换肤", 350,  20, 300, 5000)
_G._GLOVE_BOX = glove_box

local weapon_combo = gui.Combobox(skin_box, "wpn_sel", "武器", unpack(wpn_display_names))

local function get_skin_names(wpn_name)
    local names = {}
    local skins = weapon_to_skins[wpn_name]
    if skins then
        for _, s in ipairs(skins) do
            table.insert(names, s.paint_name)
        end
    end
    return names
end

local skin_combo = gui.Combobox(skin_box, "skin_sel", "皮肤", unpack(get_skin_names(weapon_list[1])))

local wear_slider = gui.Slider(skin_box, "wear_val", "磨损", 0, 0, 100, 1)
local wear_edit = gui.Editbox(skin_box, "wear_edit", "精确磨损")
local function get_wear()
    local ev = wear_edit:GetValue()
    local num = tonumber(ev)
    if num and num >= 0 and num <= 1 then return num end
    return wear_slider:GetValue() / 100.0
end
local seed_slider = gui.Slider(skin_box, "seed_val", "种子", 1, 0, 1000, 1)
gui.Button(skin_box, "应用皮肤", function() _G.APPLY_SKIN = true end)

local last_wpn_idx = -1
local auto_last_ent_idx = -1
local last_wear_sv = 0
local last_wear_ev = "0.000000"
local knife_last_reapply_t = 0
local KNIFE_REAPPLY_COOLDOWN = 0.5
callbacks.Register("Draw", "universal_skin_draw", function()
    win:SetActive(gui.Reference("Menu"):IsActive())

    local wpn_idx = weapon_combo:GetValue()
    if wpn_idx ~= last_wpn_idx then
        last_wpn_idx = wpn_idx
        local wpn_name = weapon_list[wpn_idx + 1]
        if wpn_name then
            skin_combo:SetOptions(unpack(get_skin_names(wpn_name)))
            skin_combo:SetValue(0)
        end
    end

    local pawn_aa = entities.GetLocalPawn()
    if pawn_aa and pawn_aa:IsAlive() then
        local defindex, ent_idx = get_active_weapon_defindex()
        if defindex and ent_idx and ent_idx ~= auto_last_ent_idx then
            auto_last_ent_idx = ent_idx

            local config = saved_configs[defindex]
            local use_defindex = defindex

            if not config then

                local cur_ent = entities.GetByIndex(ent_idx)
                if cur_ent and ent_is_knife(cur_ent) then
                    for def, cfg in pairs(saved_configs) do
                        if is_knife_defindex(def) then
                            config = cfg
                            use_defindex = def
                            break
                        end
            ...
            if config then
                local wpn_ptr = get_active_weapon()
                local ok = apply_auto_skin(use_defindex, wpn_ptr, config)
                if ok then
                    local name = defindex_to_wname[use_defindex] or ("#" .. use_defindex)
                    print("[自动皮肤] 已应用 " .. name)
                end
            end
        end

        local cur_ent2 = ent_idx and entities.GetByIndex(ent_idx)
        if cur_ent2 and ent_is_knife(cur_ent2) then

            local k_config, k_def = nil, nil
            for def, cfg in pairs(saved_configs) do
                if is_knife_defindex(def) then
                    k_config = cfg
                    k_def = def
            ...
            if k_config and k_def then
                local target_sub = KNIFE_DEF[k_def] and KNIFE_DEF[k_def].sub or nil

                local wpn_ptr = get_active_weapon()
                if wpn_ptr and target_sub then
                    local ok_s, cur_sub = pcall(function()
                        return ffi.cast("uint32_t*", wpn_ptr + OFF.SUBCLASS_ID)[0]
                    end)
                    if ok_s and cur_sub and tonumber(cur_sub) ~= target_sub then
                        local now = common.Time()
                        if now - knife_last_reapply_t >= KNIFE_REAPPLY_COOLDOWN then
                            knife_last_reapply_t = now
                            reapply_single_knife(wpn_ptr, k_def, k_config)
    ...
    local cur_sv = wear_slider:GetValue()
    local cur_ev = wear_edit:GetValue()
    if cur_sv ~= last_wear_sv then
        last_wear_sv = cur_sv
        last_wear_ev = string.format("%.6f", cur_sv / 100.0)
        wear_edit:SetValue(last_wear_ev)
    elseif cur_ev ~= last_wear_ev then
        last_wear_ev = cur_ev
        local num = tonumber(cur_ev)
        if num and num >= 0 and num <= 1 then
            last_wear_sv = math.floor(num * 100 + 0.5)
            wear_slider:SetValue(last_wear_sv)
        end
    end

    if _G.APPLY_SKIN then
        _G.APPLY_SKIN = false
        if not regen_addr or not fnRegen then
            print("[应用皮肤] 未找到 Regenerate 函数")
            return
        end
        if not k32 then
            print("[应用皮肤] kernel32 不可用")
            return
        end

        local wpn_idx = weapon_combo:GetValue()
        local skin_idx = skin_combo:GetValue()
        local wpn_name = weapon_list[wpn_idx + 1]
        if not wpn_name then
            print("[应用皮肤] 无效武器选择")
            return
        end
        local weapon_skins = weapon_to_skins[wpn_name]
        if not weapon_skins then
            print("[应用皮肤] 无效武器：" .. wpn_name)
            return
        end
        local skin = weapon_skins[skin_idx + 1]
        if not skin then
            print("[应用皮肤] 无效皮肤选择")
            return
        end

        local paint_id = skin.paint
        local seed_val = seed_slider:GetValue()
        local wear_val = get_wear()
        print(string.format("[应用皮肤] 应用: %s (paint=%d, seed=%d, wear=%.3f, legacy=%s)",
            skin.paint_name, paint_id, seed_val, wear_val, tostring(skin.legacy)))

        if is_knife_defindex(skin.defindex) then
            local ok_sw, msg_sw = switch_knife_type(skin.defindex)
            print(string.format("[应用皮肤][刀] 切换刀型: %s (%s)",
                ok_sw and "成功" or "失败", tostring(msg_sw)))
            if paint_id and paint_id > 0 then
                local ok_paint = apply_paint_to_all_knives(paint_id, seed_val, wear_val, skin.legacy)
                print(string.format("[应用皮肤][刀] 应用 paint=%d: %s",
                    paint_id, ok_paint and "成功" or "失败"))
            end
            save_skin_config(skin.defindex, paint_id, seed_val, wear_val, skin.legacy)
            print("[应用皮肤] 完成")
            return
        end
        ...
        local attr_addr
        local ok_attr, err = pcall(function() attr_addr = apply_ak_attributes(wpn, paint_id, seed_val, wear_val, skin.legacy) end)
        if not ok_attr then
            print("[应用皮肤] 属性修改失败: "..tostring(err))
            return
        end
        local before = u16(regen_addr + 0x52)
        local ok_patch, info = protect_write(regen_addr + 0x52, 2, EXPECTED_IMM)
        local after = u16(regen_addr + 0x52)
        if not ok_patch or after ~= EXPECTED_IMM then
            print("[应用皮肤] Patch 失败，跳过 Regenerate")
            cleanup_attributes(wpn, attr_addr)
            return
        end
        local ok_regen, err_regen = pcall(fnRegen)
        print(string.format("[应用皮肤] Regenerate: %s", ok_regen and "成功" or tostring(err_regen)))
        protect_write(regen_addr + 0x52, 2, ORIGINAL_IMM)
        cleanup_attributes(wpn, attr_addr)
...
print("[通用皮肤] 脚本已加载。请确保持有枪械，并选择皮肤后点击应用。")
print("[通用皮肤] 皮肤列表已加载，共 " .. #all_skins .. " 个皮肤（含刀），涵盖 " .. #weapon_list .. " 种武器。")





local SIG_NEW = "40 53 48 83 EC ?? 48 8B D9 4C 8B C2 48 8B 0D ?? ?? ?? ?? 48 8D 54 24 40"
local SIG_OLD = "40 53 48 83 EC 20 48 8B D9 4C 8B C2 48 8B 0D ?? ?? ?? ?? 48 8D 54 24"

local addr_new = mem.FindPattern("client.dll", SIG_NEW)
local addr_old = mem.FindPattern("client.dll", SIG_OLD)

print(string.format("[探员] SetModel 新签名地址: 0x%X", tonumber(addr_new or 0)))
print(string.format("[探员] SetModel 旧签名地址: 0x%X", tonumber(addr_old or 0)))


local setmodel_addr = addr_new or addr_old
if not setmodel_addr or setmodel_addr == 0 then
    print("[探员] 两个签名都找不到，退出。")
    return
end
if addr_new then
    print("[探员] 使用新签名")
...
local fnSetModel_agent = ffi.cast("void*(__thiscall*)(void*, const char*)", setmodel_addr)


local _path_cache_agent = {}
local function cstr_path_agent(path)
    if not _path_cache_agent[path] then
        local s = ffi.new("char[?]", #path + 1)
        ffi.copy(s, path)
        _path_cache_agent[path] = s
    end
    return _path_cache_agent[path]
end


local AGENTS = {
    { id=4613, team="T", name="customplayer_tm_professional_varf5", path="agents/models/tm_professional/tm_professional_varf5.vmdl", label="残酷的达里尔（穷鬼）| 专业人士" },
    { id=4718, team="T", name="customplayer_tm_balkan_variantk", path="agents/models/tm_balkan/tm_balkan_variantk.vmdl", label="红衫列赞 | 军刀" },
    { id=4726, team="T", name="customplayer_tm_professional_varf", path="agents/models/tm_professional/tm_professional_varf.vmdl", label="残酷的达里尔爵士（迈阿密）| 专业人士" },
    { id=4727, team="T", name="customplayer_tm_professional_varg", path="agents/models/tm_professional/tm_professional_varg.vmdl", label="飞贼波兹曼 | 专业人士" },
    { id=4728, team="T", name="customplayer_tm_professional_varh", path="agents/models/tm_professional/tm_professional_varh.vmdl", label="小凯夫 | 专业人士" },
    { id=4730, team="T", name="customplayer_tm_professional_varj", path="agents/models/tm_professional/tm_professional_varj.vmdl", label="出逃的萨莉 | 专业人士" },
    { id=4732, team="T", name="customplayer_tm_professional_vari", path="agents/models/tm_professional/tm_professional_vari.vmdl", label="老K | 专业人士" },
    { id=4733, team="T", name="customplayer_tm_professional_varf1", path="agents/models/tm_professional/tm_professional_varf1.vmdl", label="残酷的达里尔爵士（沉默）| 专业人士" },
    { id=4734, team="T", name="customplayer_tm_professional_varf2", path="agents/models/tm_professional/tm_professional_varf2.vmdl", label="残酷的达里尔爵士（头盖骨）| 专业人士" },
    { id=4735, team="T", name="customplayer_tm_professional_varf3", path="agents/models/tm_professional/tm_professional_varf3.vmdl", label="残酷的达里尔爵士（皇家）| 专业人士" },
    { id=4736, team="T", name="customplayer_tm_professional_varf4", path="agents/models/tm_professional/tm_professional_varf4.vmdl", label="残酷的达里尔爵士（聒噪）| 专业人士" },
    { id=4773, team="T", name="customplayer_tm_jungle_raider_varianta", path="agents/models/tm_jungle_raider/tm_jungle_raider_varianta.vmdl", label="精锐捕兽者索尔曼 | 游击队" },
    { id=4774, team="T", name="customplayer_tm_jungle_raider_variantb", path="agents/models/tm_jungle_raider/tm_jungle_raider_variantb.vmdl", label="遗忘者克拉斯沃特 | 游击队" },
    { id=4775, team="T", name="customplayer_tm_jungle_raider_variantc", path="agents/models/tm_jungle_raider/tm_jungle_raider_variantc.vmdl", label="亚诺（野草） | 游击队" },
    { id=4776, team="T", name="customplayer_tm_jungle_raider_variantd", path="agents/models/tm_jungle_raider/tm_jungle_raider_variantd.vmdl", label="上校曼戈斯·达比西 | 游击队" },
    { id=4777, team="T", name="customplayer_tm_jungle_raider_variante", path="agents/models/tm_jungle_raider/tm_jungle_raider_variante.vmdl", label="薇帕姐（革新派） | 游击队" },
    { id=4778, team="T", name="customplayer_tm_jungle_raider_variantf", path="agents/models/tm_jungle_raider/tm_jungle_raider_variantf.vmdl", label="捕兽者（挑衅者） | 游击队" },
    { id=4780, team="T", name="customplayer_tm_jungle_raider_variantb2", path="agents/models/tm_jungle_raider/tm_jungle_raider_variantb2.vmdl", label="克拉斯沃特（三分熟） | 游击队" },
    { id=4781, team="T", name="customplayer_tm_jungle_raider_variantf2", path="agents/models/tm_jungle_raider/tm_jungle_raider_variantf2.vmdl", label="捕兽者 | 游击队" },
    { id=5038, team="T", name="customplayer_tm_anarchist", path="agents/models/tm_anarchist/tm_anarchist.vmdl", label="[T] 无政府 · 普通" },
    { id=5039, team="T", name="customplayer_tm_anarchist_varianta", path="agents/models/tm_anarchist/tm_anarchist_varianta.vmdl", label="[T] 无政府 (varianta) · 普通" },
    { id=5040, team="T", name="customplayer_tm_anarchist_variantb", path="agents/models/tm_anarchist/tm_anarchist_variantb.vmdl", label="[T] 无政府 (variantb) · 普通" },
    { id=5041, team="T", name="customplayer_tm_anarchist_variantc", path="agents/models/tm_anarchist/tm_anarchist_variantc.vmdl", label="[T] 无政府 (variantc) · 普通" },
    { id=5042, team="T", name="customplayer_tm_anarchist_variantd", path="agents/models/tm_anarchist/tm_anarchist_variantd.vmdl", label="[T] 无政府 (variantd) · 普通" },
    { id=5043, team="T", name="customplayer_tm_pirate", path="agents/models/tm_pirate/tm_pirate.vmdl", label="[T] 海盗 · 普通" },
    { id=5044, team="T", name="customplayer_tm_pirate_varianta", path="agents/models/tm_pirate/tm_pirate_varianta.vmdl", label="[T] 海盗 (varianta) · 普通" },
    { id=5045, team="T", name="customplayer_tm_pirate_variantb", path="agents/models/tm_pirate/tm_pirate_variantb.vmdl", label="[T] 海盗 (variantb) · 普通" },
    { id=5046, team="T", name="customplayer_tm_pirate_variantc", path="agents/models/tm_pirate/tm_pirate_variantc.vmdl", label="[T] 海盗 (variantc) · 普通" },
    { id=5047, team="T", name="customplayer_tm_pirate_variantd", path="agents/models/tm_pirate/tm_pirate_variantd.vmdl", label="[T] 海盗 (variantd) · 普通" },
    { id=5048, team="T", name="customplayer_tm_professional", path="agents/models/tm_professional/tm_professional.vmdl", label="[T] 职业杀手 · 普通" },
    { id=5049, team="T", name="customplayer_tm_professional_var1", path="agents/models/tm_professional/tm_professional_var1.vmdl", label="[T] 职业杀手 (var1) · 普通" },
    { id=5050, team="T", name="customplayer_tm_professional_var2", path="agents/models/tm_professional/tm_professional_var2.vmdl", label="[T] 职业杀手 (var2) · 普通" },
    { id=5051, team="T", name="customplayer_tm_professional_var3", path="agents/models/tm_professional/tm_professional_var3.vmdl", label="[T] 职业杀手 (var3) · 普通" },
    { id=5052, team="T", name="customplayer_tm_professional_var4", path="agents/models/tm_professional/tm_professional_var4.vmdl", label="[T] 职业杀手 (var4) · 普通" },
    { id=5053, team="T", name="customplayer_tm_separatist", path="agents/models/tm_separatist/tm_separatist.vmdl", label="[T] 分离主义 · 普通" },
    { id=5054, team="T", name="customplayer_tm_separatist_varianta", path="agents/models/tm_separatist/tm_separatist_varianta.vmdl", label="[T] 分离主义 (varianta) · 普通" },
    { id=5055, team="T", name="customplayer_tm_separatist_variantb", path="agents/models/tm_separatist/tm_separatist_variantb.vmdl", label="[T] 分离主义 (variantb) · 普通" },
    { id=5056, team="T", name="customplayer_tm_separatist_variantc", path="agents/models/tm_separatist/tm_separatist_variantc.vmdl", label="[T] 分离主义 (variantc) · 普通" },
    { id=5057, team="T", name="customplayer_tm_separatist_variantd", path="agents/models/tm_separatist/tm_separatist_variantd.vmdl", label="[T] 分离主义 (variantd) · 普通" },
    { id=5088, team="T", name="customplayer_tm_balkan_variante", path="agents/models/tm_balkan/tm_balkan_variante.vmdl", label="[T] 巴尔干 (variante) · 普通" },
    { id=5089, team="T", name="customplayer_tm_balkan_varianta", path="agents/models/tm_balkan/tm_balkan_varianta.vmdl", label="[T] 巴尔干 (varianta) · 普通" },
    { id=5090, team="T", name="customplayer_tm_balkan_variantb", path="agents/models/tm_balkan/tm_balkan_variantb.vmdl", label="[T] 巴尔干 (variantb) · 普通" },
    { id=5091, team="T", name="customplayer_tm_balkan_variantc", path="agents/models/tm_balkan/tm_balkan_variantc.vmdl", label="[T] 巴尔干 (variantc) · 普通" },
    { id=5092, team="T", name="customplayer_tm_balkan_variantd", path="agents/models/tm_balkan/tm_balkan_variantd.vmdl", label="[T] 巴尔干 (variantd) · 普通" },
    { id=5093, team="T", name="customplayer_tm_jumpsuit_varianta", path="agents/models/tm_jumpsuit/tm_jumpsuit_varianta.vmdl", label="[T] jumpsuit (varianta) · 普通" },
    { id=5094, team="T", name="customplayer_tm_jumpsuit_variantb", path="agents/models/tm_jumpsuit/tm_jumpsuit_variantb.vmdl", label="[T] jumpsuit (variantb) · 普通" },
    { id=5095, team="T", name="customplayer_tm_jumpsuit_variantc", path="agents/models/tm_jumpsuit/tm_jumpsuit_variantc.vmdl", label="[T] jumpsuit (variantc) · 普通" },
    { id=5096, team="T", name="customplayer_tm_phoenix_heavy", path="agents/models/tm_phoenix_heavy/tm_phoenix_heavy.vmdl", label="[T] phoenix_heavy · 普通" },
    { id=5100, team="T", name="customplayer_tm_leet_varianta", path="agents/models/tm_leet/tm_leet_varianta.vmdl", label="[T] 精英 (varianta) · 普通" },
    { id=5101, team="T", name="customplayer_tm_leet_variantb", path="agents/models/tm_leet/tm_leet_variantb.vmdl", label="[T] 精英 (variantb) · 普通" },
    { id=5102, team="T", name="customplayer_tm_leet_variantc", path="agents/models/tm_leet/tm_leet_variantc.vmdl", label="[T] 精英 (variantc) · 普通" },
    { id=5103, team="T", name="customplayer_tm_leet_variantd", path="agents/models/tm_leet/tm_leet_variantd.vmdl", label="[T] 精英 (variantd) · 普通" },
    { id=5104, team="T", name="customplayer_tm_leet_variante", path="agents/models/tm_leet/tm_leet_variante.vmdl", label="[T] 精英 (variante) · 普通" },
    { id=5105, team="T", name="customplayer_tm_leet_variantg", path="agents/models/tm_leet/tm_leet_variantg.vmdl", label="地面叛军 | 精锐分子" },
    { id=5106, team="T", name="customplayer_tm_leet_varianth", path="agents/models/tm_leet/tm_leet_varianth.vmdl", label="奥西瑞斯 | 精锐分子" },
    { id=5107, team="T", name="customplayer_tm_leet_varianti", path="agents/models/tm_leet/tm_leet_varianti.vmdl", label="沙哈马特教授 | 精锐分子" },
    { id=5108, team="T", name="customplayer_tm_leet_variantf", path="agents/models/tm_leet/tm_leet_variantf.vmdl", label="精英穆哈里克先生 | 精锐分子" },
    { id=5109, team="T", name="customplayer_tm_leet_variantj", path="agents/models/tm_leet/tm_leet_variantj.vmdl", label="丛林反抗者 | 精锐分子" },
    { id=5200, team="T", name="customplayer_tm_phoenix", path="agents/models/tm_phoenix/tm_phoenix.vmdl", label="[T] 凤凰 · 普通" },
    { id=5201, team="T", name="customplayer_tm_phoenix_varianta", path="agents/models/tm_phoenix/tm_phoenix_varianta.vmdl", label="[T] 凤凰 (varianta) · 普通" },
    { id=5202, team="T", name="customplayer_tm_phoenix_variantb", path="agents/models/tm_phoenix/tm_phoenix_variantb.vmdl", label="[T] 凤凰 (variantb) · 普通" },
    { id=5203, team="T", name="customplayer_tm_phoenix_variantc", path="agents/models/tm_phoenix/tm_phoenix_variantc.vmdl", label="[T] 凤凰 (variantc) · 普通" },
    { id=5204, team="T", name="customplayer_tm_phoenix_variantd", path="agents/models/tm_phoenix/tm_phoenix_variantd.vmdl", label="[T] 凤凰 (variantd) · 普通" },
    { id=5205, team="T", name="customplayer_tm_phoenix_varianth", path="agents/models/tm_phoenix/tm_phoenix_varianth.vmdl", label="枪手 | 凤凰战士" },
    { id=5206, team="T", name="customplayer_tm_phoenix_variantf", path="agents/models/tm_phoenix/tm_phoenix_variantf.vmdl", label="执行者 | 凤凰战士" },
    { id=5207, team="T", name="customplayer_tm_phoenix_variantg", path="agents/models/tm_phoenix/tm_phoenix_variantg.vmdl", label="弹弓 | 凤凰战士" },
    { id=5208, team="T", name="customplayer_tm_phoenix_varianti", path="agents/models/tm_phoenix/tm_phoenix_varianti.vmdl", label="街头士兵 | 凤凰战士" },
    { id=5500, team="T", name="customplayer_tm_balkan_variantf", path="agents/models/tm_balkan/tm_balkan_variantf.vmdl", label="德拉戈米尔 | 军刀" },
    { id=5501, team="T", name="customplayer_tm_balkan_varianti", path="agents/models/tm_balkan/tm_balkan_varianti.vmdl", label="马克西姆斯 | 军刀" },
    { id=5502, team="T", name="customplayer_tm_balkan_variantg", path="agents/models/tm_balkan/tm_balkan_variantg.vmdl", label="准备就绪的列赞 | 军刀" },
    { id=5503, team="T", name="customplayer_tm_balkan_variantj", path="agents/models/tm_balkan/tm_balkan_variantj.vmdl", label="黑狼 | 军刀" },
    { id=5504, team="T", name="customplayer_tm_balkan_varianth", path="agents/models/tm_balkan/tm_balkan_varianth.vmdl", label="“医生”罗曼诺夫 | 军刀" },
    { id=5505, team="T", name="customplayer_tm_balkan_variantl", path="agents/models/tm_balkan/tm_balkan_variantl.vmdl", label="德拉戈米尔 | 军刀勇士" },
    { id=4619, team="CT", name="customplayer_ctm_st6_variantj", path="agents/models/ctm_st6/ctm_st6_variantj.vmdl", label="“蓝莓” 铅弹 | 海军水面战中心海豹部队" },
    { id=4680, team="CT", name="customplayer_ctm_st6_variantl", path="agents/models/ctm_st6/ctm_st6_variantl.vmdl", label="“两次”麦考伊 | 战术空中管制部队装甲兵" },
    { id=4711, team="CT", name="customplayer_ctm_swat_variante", path="agents/models/ctm_swat/ctm_swat_variante.vmdl", label="指挥官 梅 “极寒” 贾米森 | 特警" },
    { id=4712, team="CT", name="customplayer_ctm_swat_variantf", path="agents/models/ctm_swat/ctm_swat_variantf.vmdl", label="第一中尉法洛 | 特警" },
    { id=4713, team="CT", name="customplayer_ctm_swat_variantg", path="agents/models/ctm_swat/ctm_swat_variantg.vmdl", label="约翰 “范·海伦” 卡斯克 | 特警" },
    { id=4714, team="CT", name="customplayer_ctm_swat_varianth", path="agents/models/ctm_swat/ctm_swat_varianth.vmdl", label="生物防害专家 | 特警" },
    { id=4715, team="CT", name="customplayer_ctm_swat_varianti", path="agents/models/ctm_swat/ctm_swat_varianti.vmdl", label="军士长炸弹森 | 特警" },
    { id=4716, team="CT", name="customplayer_ctm_swat_variantj", path="agents/models/ctm_swat/ctm_swat_variantj.vmdl", label="化学防害专家 | 特警" },
    { id=4749, team="CT", name="customplayer_ctm_gendarmerie_varianta", path="agents/models/ctm_gendarmerie/ctm_gendarmerie_varianta.vmdl", label="军医少尉 | 法国宪兵特勤队" },
    { id=4750, team="CT", name="customplayer_ctm_gendarmerie_variantb", path="agents/models/ctm_gendarmerie/ctm_gendarmerie_variantb.vmdl", label="化学防害上尉| 法国宪兵特勤队" },
    { id=4751, team="CT", name="customplayer_ctm_gendarmerie_variantc", path="agents/models/ctm_gendarmerie/ctm_gendarmerie_variantc.vmdl", label="中队长鲁沙尔·勒库托 | 法国宪兵特勤队" },
    { id=4752, team="CT", name="customplayer_ctm_gendarmerie_variantd", path="agents/models/ctm_gendarmerie/ctm_gendarmerie_variantd.vmdl", label="准尉 | 法国宪兵特勤队" },
    { id=4753, team="CT", name="customplayer_ctm_gendarmerie_variante", path="agents/models/ctm_gendarmerie/ctm_gendarmerie_variante.vmdl", label="军官雅克·贝尔特朗 | 法国宪兵特勤队" },
    { id=4756, team="CT", name="customplayer_ctm_swat_variantk", path="agents/models/ctm_swat/ctm_swat_variantk.vmdl", label="中尉法洛（抱树人） | 特警" },
    { id=4757, team="CT", name="customplayer_ctm_diver_varianta", path="agents/models/ctm_diver/ctm_diver_varianta.vmdl", label="指挥官黛维达·费尔南德斯（护目镜） | 海豹蛙人" },
    { id=4771, team="CT", name="customplayer_ctm_diver_variantb", path="agents/models/ctm_diver/ctm_diver_variantb.vmdl", label="指挥官弗兰克·巴鲁德（湿袜） | 海豹蛙人" },
    { id=4772, team="CT", name="customplayer_ctm_diver_variantc", path="agents/models/ctm_diver/ctm_diver_variantc.vmdl", label="中尉雷克斯·克里奇 | 海豹蛙人" },
    { id=5058, team="CT", name="customplayer_ctm_gign", path="agents/models/ctm_gign/ctm_gign.vmdl", label="[CT] GIGN · 普通" },
    { id=5059, team="CT", name="customplayer_ctm_gign_varianta", path="agents/models/ctm_gign/ctm_gign_varianta.vmdl", label="[CT] GIGN (varianta) · 普通" },
    { id=5060, team="CT", name="customplayer_ctm_gign_variantb", path="agents/models/ctm_gign/ctm_gign_variantb.vmdl", label="[CT] GIGN (variantb) · 普通" },
    { id=5061, team="CT", name="customplayer_ctm_gign_variantc", path="agents/models/ctm_gign/ctm_gign_variantc.vmdl", label="[CT] GIGN (variantc) · 普通" },
    { id=5062, team="CT", name="customplayer_ctm_gign_variantd", path="agents/models/ctm_gign/ctm_gign_variantd.vmdl", label="[CT] GIGN (variantd) · 普通" },
    { id=5063, team="CT", name="customplayer_ctm_gsg9", path="agents/models/ctm_gsg9/ctm_gsg9.vmdl", label="[CT] GSG9 · 普通" },
    { id=5064, team="CT", name="customplayer_ctm_gsg9_varianta", path="agents/models/ctm_gsg9/ctm_gsg9_varianta.vmdl", label="[CT] GSG9 (varianta) · 普通" },
    { id=5065, team="CT", name="customplayer_ctm_gsg9_variantb", path="agents/models/ctm_gsg9/ctm_gsg9_variantb.vmdl", label="[CT] GSG9 (variantb) · 普通" },
    { id=5066, team="CT", name="customplayer_ctm_gsg9_variantc", path="agents/models/ctm_gsg9/ctm_gsg9_variantc.vmdl", label="[CT] GSG9 (variantc) · 普通" },
    { id=5067, team="CT", name="customplayer_ctm_gsg9_variantd", path="agents/models/ctm_gsg9/ctm_gsg9_variantd.vmdl", label="[CT] GSG9 (variantd) · 普通" },
    { id=5068, team="CT", name="customplayer_ctm_idf", path="agents/models/ctm_idf/ctm_idf.vmdl", label="[CT] idf · 普通" },
    { id=5069, team="CT", name="customplayer_ctm_idf_variantb", path="agents/models/ctm_idf/ctm_idf_variantb.vmdl", label="[CT] idf (variantb) · 普通" },
    { id=5070, team="CT", name="customplayer_ctm_idf_variantc", path="agents/models/ctm_idf/ctm_idf_variantc.vmdl", label="[CT] idf (variantc) · 普通" },
    { id=5071, team="CT", name="customplayer_ctm_idf_variantd", path="agents/models/ctm_idf/ctm_idf_variantd.vmdl", label="[CT] idf (variantd) · 普通" },
    { id=5072, team="CT", name="customplayer_ctm_idf_variante", path="agents/models/ctm_idf/ctm_idf_variante.vmdl", label="[CT] idf (variante) · 普通" },
    { id=5073, team="CT", name="customplayer_ctm_idf_variantf", path="agents/models/ctm_idf/ctm_idf_variantf.vmdl", label="[CT] idf (variantf) · 普通" },
    { id=5074, team="CT", name="customplayer_ctm_swat", path="agents/models/ctm_swat/ctm_swat.vmdl", label="[CT] SWAT · 普通" },
    { id=5075, team="CT", name="customplayer_ctm_swat_varianta", path="agents/models/ctm_swat/ctm_swat_varianta.vmdl", label="[CT] SWAT (varianta) · 普通" },
    { id=5076, team="CT", name="customplayer_ctm_swat_variantb", path="agents/models/ctm_swat/ctm_swat_variantb.vmdl", label="[CT] SWAT (variantb) · 普通" },
    { id=5077, team="CT", name="customplayer_ctm_swat_variantc", path="agents/models/ctm_swat/ctm_swat_variantc.vmdl", label="[CT] SWAT (variantc) · 普通" },
    { id=5078, team="CT", name="customplayer_ctm_swat_variantd", path="agents/models/ctm_swat/ctm_swat_variantd.vmdl", label="[CT] SWAT (variantd) · 普通" },
    { id=5079, team="CT", name="customplayer_ctm_sas_varianta", path="agents/models/ctm_sas/ctm_sas_varianta.vmdl", label="[CT] SAS (varianta) · 普通" },
    { id=5080, team="CT", name="customplayer_ctm_sas_variantb", path="agents/models/ctm_sas/ctm_sas_variantb.vmdl", label="[CT] SAS (variantb) · 普通" },
    { id=5081, team="CT", name="customplayer_ctm_sas_variantc", path="agents/models/ctm_sas/ctm_sas_variantc.vmdl", label="[CT] SAS (variantc) · 普通" },
    { id=5082, team="CT", name="customplayer_ctm_sas_variantd", path="agents/models/ctm_sas/ctm_sas_variantd.vmdl", label="[CT] SAS (variantd) · 普通" },
    { id=5083, team="CT", name="customplayer_ctm_st6", path="agents/models/ctm_st6/ctm_st6.vmdl", label="[CT] ST6 · 普通" },
    { id=5084, team="CT", name="customplayer_ctm_st6_varianta", path="agents/models/ctm_st6/ctm_st6_varianta.vmdl", label="[CT] ST6 (varianta) · 普通" },
    { id=5085, team="CT", name="customplayer_ctm_st6_variantb", path="agents/models/ctm_st6/ctm_st6_variantb.vmdl", label="[CT] ST6 (variantb) · 普通" },
    { id=5086, team="CT", name="customplayer_ctm_st6_variantc", path="agents/models/ctm_st6/ctm_st6_variantc.vmdl", label="[CT] ST6 (variantc) · 普通" },
    { id=5087, team="CT", name="customplayer_ctm_st6_variantd", path="agents/models/ctm_st6/ctm_st6_variantd.vmdl", label="[CT] ST6 (variantd) · 普通" },
    { id=5097, team="CT", name="customplayer_ctm_heavy", path="agents/models/ctm_heavy/ctm_heavy.vmdl", label="[CT] heavy · 普通" },
    { id=5300, team="CT", name="customplayer_ctm_fbi", path="agents/models/ctm_fbi/ctm_fbi.vmdl", label="[CT] FBI · 普通" },
    { id=5301, team="CT", name="customplayer_ctm_fbi_varianta", path="agents/models/ctm_fbi/ctm_fbi_varianta.vmdl", label="[CT] FBI (varianta) · 普通" },
    { id=5302, team="CT", name="customplayer_ctm_fbi_variantc", path="agents/models/ctm_fbi/ctm_fbi_variantc.vmdl", label="[CT] FBI (variantc) · 普通" },
    { id=5303, team="CT", name="customplayer_ctm_fbi_variantd", path="agents/models/ctm_fbi/ctm_fbi_variantd.vmdl", label="[CT] FBI (variantd) · 普通" },
    { id=5304, team="CT", name="customplayer_ctm_fbi_variante", path="agents/models/ctm_fbi/ctm_fbi_variante.vmdl", label="[CT] FBI (variante) · 普通" },
    { id=5305, team="CT", name="customplayer_ctm_fbi_variantf", path="agents/models/ctm_fbi/ctm_fbi_variantf.vmdl", label="特种兵 | 联邦调查局（FBI）特警" },
    { id=5306, team="CT", name="customplayer_ctm_fbi_variantg", path="agents/models/ctm_fbi/ctm_fbi_variantg.vmdl", label="马尔库斯·戴劳 | 联邦调查局（FBI）人质营救队" },
    { id=5307, team="CT", name="customplayer_ctm_fbi_varianth", path="agents/models/ctm_fbi/ctm_fbi_varianth.vmdl", label="迈克·赛弗斯 | 联邦调查局（FBI）狙击手" },
    { id=5308, team="CT", name="customplayer_ctm_fbi_variantb", path="agents/models/ctm_fbi/ctm_fbi_variantb.vmdl", label="爱娃特工 | 联邦调查局（FBI）" },
    { id=5400, team="CT", name="customplayer_ctm_st6_variantk", path="agents/models/ctm_st6/ctm_st6_variantk.vmdl", label="第三特种兵连 | 德国特种部队突击队" },
    { id=5401, team="CT", name="customplayer_ctm_st6_variante", path="agents/models/ctm_st6/ctm_st6_variante.vmdl", label="海豹突击队第六分队士兵 | 海军水面战中心海豹部队" },
    { id=5402, team="CT", name="customplayer_ctm_st6_variantg", path="agents/models/ctm_st6/ctm_st6_variantg.vmdl", label="铅弹 | 海军水面战中心海豹部队" },
    { id=5403, team="CT", name="customplayer_ctm_st6_variantm", path="agents/models/ctm_st6/ctm_st6_variantm.vmdl", label="“两次”麦考伊 | 美国空军战术空中管制部队" },
    { id=5404, team="CT", name="customplayer_ctm_st6_varianti", path="agents/models/ctm_st6/ctm_st6_varianti.vmdl", label="海军上尉里克索尔 | 海军水面战中心海豹部队" },
    { id=5405, team="CT", name="customplayer_ctm_st6_variantn", path="agents/models/ctm_st6/ctm_st6_variantn.vmdl", label="陆军中尉普里米罗 | 巴西第一营" },
    { id=5600, team="CT", name="customplayer_ctm_sas", path="agents/models/ctm_sas/ctm_sas.vmdl", label="[CT] SAS · 普通" },
    { id=5601, team="CT", name="customplayer_ctm_sas_variantf", path="agents/models/ctm_sas/ctm_sas_variantf.vmdl", label="B 中队指挥官 | 英国空军特别部队" },
    { id=5602, team="CT", name="customplayer_ctm_sas_variantg", path="agents/models/ctm_sas/ctm_sas_variantg.vmdl", label="D 中队军官 | 新西兰特种空勤团" },
}


local function partition()
    local all, t_only, ct_only = {}, {}, {}
    local L_all, L_t, L_ct = {}, {}, {}
    for _, a in ipairs(AGENTS) do
        all[#all+1] = a; L_all[#L_all+1] = a.label
        if a.team == "T" then
            t_only[#t_only+1] = a; L_t[#L_t+1] = a.label
        else
            ct_only[#ct_only+1] = a; L_ct[#L_ct+1] = a.label
        end
    end
    return all, L_all, t_only, L_t, ct_only, L_ct
end

local A_ALL, L_ALL, A_T, L_T, A_CT, L_CT = partition()
print(string.format("[探员] 已加载 %d 个 (T=%d CT=%d)", #A_ALL, #A_T, #A_CT))

local active_list, active_labels = A_ALL, L_ALL
...
local origin_model   = nil



local AGENT_CONFIG_FILE = "agent_config.txt"

_G._AGENT_SAVED = _G._AGENT_SAVED or { T = nil, CT = nil }

function _G._AGENT_SAVE_TO_FILE()
    local lines = {}
    if _G._AGENT_SAVED.T   then lines[#lines+1] = "T="  .. _G._AGENT_SAVED.T.path  .. "|" .. (_G._AGENT_SAVED.T.label  or "") end
    if _G._AGENT_SAVED.CT  then lines[#lines+1] = "CT=" .. _G._AGENT_SAVED.CT.path .. "|" .. (_G._AGENT_SAVED.CT.label or "") end
    pcall(file.Write, AGENT_CONFIG_FILE, table.concat(lines, "\n"))
end

function _G._AGENT_LOAD_FROM_FILE()
    local ok, content = pcall(file.Read, AGENT_CONFIG_FILE)
    if not ok or not content or content == "" then return end
    for line in content:gmatch("[^\r\n]+") do
        local team, rest = line:match("^(%w+)=(.+)$")
        if team and rest then
            local path, label = rest:match("^([^|]+)|(.*)$")
            if not path then path = rest end
            if team == "T" or team == "CT" then
                _G._AGENT_SAVED[team] = { path = path, label = label or path }
            end
        end
    end
    print(string.format("[探员] 已加载配置 T=%s  CT=%s",
        _G._AGENT_SAVED.T and _G._AGENT_SAVED.T.label or "(无)",
        _G._AGENT_SAVED.CT and _G._AGENT_SAVED.CT.label or "(无)"))
end

_G._AGENT_LOAD_FROM_FILE()



gui.Text(skin_box, "")

local agent_box = gui.Groupbox(win, "探员更换", 15, 330, 300, 300)

local team_combo  = gui.Combobox(agent_box, "agent_team", "阵营",
    "全部", "仅 T", "仅 CT")
local agent_combo = gui.Combobox(agent_box, "agent_sel", "探员",
    unpack(L_ALL))

local last_filter = 0
callbacks.Register("Draw", "agent_v4_filter", function()
    local f = team_combo:GetValue()
    if f ~= last_filter then
        last_filter = f
        if f == 0 then active_list, active_labels = A_ALL, L_ALL
        elseif f == 1 then active_list, active_labels = A_T, L_T
        else active_list, active_labels = A_CT, L_CT end
        agent_combo:SetOptions(unpack(active_labels))
        agent_combo:SetValue(0)
    end
end)


local function do_set_model_now(path, label)
    local pawn = entities.GetLocalPawn()
    if not pawn then
        print("[探员] no local pawn")
        return false
    end
    if not pawn:IsAlive() then
        print("[探员] pawn not alive")
        return false
    end
    local idx = pawn:GetIndex()
    local inst = GetEntityInstance(idx)
    ...
    if origin_model == nil then
        local ok, m = pcall(function() return pawn:GetModelName() end)
        if ok and m and m ~= "" then
            origin_model = m
            print(string.format("[探员] 记录原始模型: %s", m))
        end
    end

    print(string.format("[探员][FrameStage6] 应用 [%s] -> %s", label, path))
    local cs = cstr_path_agent(path)
    local ok = pcall(fnSetModel_agent, ffi.cast("void*", inst), cs)
    print(string.format("[探员] SetModel 返回: %s", ok and "OK" or "FAIL"))


    local ok_v, after = pcall(function() return pawn:GetModelName() end)
    if ok_v then
        local hit_err = (after == "models/dev/error.vmdl")
        print(string.format("[探员] 应用后模型: %s%s",
            tostring(after), hit_err and "  (路径不存在!)" or ""))
        return not hit_err
    end
    return ok
end
...
callbacks.Register("Draw", "agent_v4_apply", function()
    ...
    if action == "apply" then
        local ok = do_set_model_now(path, label)
        if ok then

            local pawn = entities.GetLocalPawn()
            if pawn then
                local team = pawn:GetTeamNumber()
                local key = (team == 2) and "T" or (team == 3) and "CT" or nil
                if key then
                    _G._AGENT_SAVED[key] = { path = path, label = label }
                    _G._AGENT_SAVE_TO_FILE()
                    print(string.format("[探员] 已保存到配置 (%s 队): %s", key, label))
                end
            end
        end
    elseif action == "restore" then
        if origin_model then
            do_set_model_now(origin_model, "原始")
            ...
            if pawn then
                ...
                local key = (team == 2) and "T" or (team == 3) and "CT" or nil
                if key then
                    _G._AGENT_SAVED[key] = nil
                    _G._AGENT_SAVE_TO_FILE()
                    print(string.format("[探员] 已清除 %s 队配置", key))
                end
            end
        else
            print("[探员] 还没有记录到原始模型")
...
local function selected_agent()
    return active_list[agent_combo:GetValue() + 1]
end


gui.Button(agent_box, "应用所选探员", function()
    local t = selected_agent()
    if not t then print("[探员] 选择无效"); return end
    pending_action = "apply"
    pending_path   = t.path
    pending_label  = t.label
    print(string.format("[探员] 已排程 (下一帧执行): %s", t.label))
end)

gui.Button(agent_box, "恢复原模型", function()
    pending_action = "restore"
    print("[探员] 已排程恢复")
end)

gui.Button(agent_box, "打印 pawn 信息", function()
    local pawn = entities.GetLocalPawn()
    if not pawn then print("[探员] no pawn"); return end
    print(string.format("[探员] idx=%d team=%d alive=%s",
        pawn:GetIndex(), pawn:GetTeamNumber(),
        tostring(pawn:IsAlive())))
    print(string.format("[探员] 当前模型: %s",
        tostring(pawn:GetModelName())))
end)
...
_G._AGENT_LAST_APPLY_T = _G._AGENT_LAST_APPLY_T or 0
_G._AGENT_PERSIST_DEBUG_LAST = _G._AGENT_PERSIST_DEBUG_LAST or 0
_G._AGENT_PERSIST_DEBUG_LAST_TEAM = _G._AGENT_PERSIST_DEBUG_LAST_TEAM or -1
_G._AGENT_PERSIST_APPLY_COUNT = _G._AGENT_PERSIST_APPLY_COUNT or 0

callbacks.Register("Draw", "agent_v4_persist", function()
    if not fnSetModel_agent then return end
    local pawn = entities.GetLocalPawn()
    if not pawn or not pawn:IsAlive() then return end

    local team = pawn:GetTeamNumber()
    local cfg = (team == 2) and _G._AGENT_SAVED.T or (team == 3) and _G._AGENT_SAVED.CT or nil
    if not cfg or not cfg.path or cfg.path == "" then return end

    local cur_model = nil
    local ok_m, m = pcall(function() return pawn:GetModelName() end)
    if ok_m then cur_model = m end
    if cur_model == cfg.path then return end


    local now = common.Time()
    if now - _G._AGENT_LAST_APPLY_T < 0.5 then return end
    _G._AGENT_LAST_APPLY_T = now

    local idx = pawn:GetIndex()
    local inst = GetEntityInstance(idx)
    if not inst or inst == 0 then return end

    local cs = cstr_path_agent(cfg.path)
    pcall(fnSetModel_agent, ffi.cast("void*", inst), cs)


    print(string.format("[探员] 自动应用 %s 队探员: %s", (team == 2) and "T" or "CT", cfg.label or cfg.path))
end)

print("[探员] 模块已加载，与枪皮共用容器")
...
local OFF_GLV_M_ECON_GLOVES   = 0x1658
local OFF_GLV_M_BNEED_REAPPLY = 0x1655
local OFF_GLV_DEF_IDX         = 0x1BA
local OFF_GLV_ENTITY_QUALITY  = 0x1BC
local OFF_GLV_INITIALIZED     = 0x1E8
local OFF_GLV_ATTR_LIST       = 0x208
local OFF_GLV_ATTRIBUTES      = 0x08
local GLV_QUALITY_UNUSUAL = 4
local GLV_ATTR_PAINT      = 6
local GLV_ATTR_PATTERN    = 7
local GLV_ATTR_WEAR       = 8
local GLV_ATTR_SIZE       = 0x48
local GLV_ATTR_DEF_OFF    = 0x30
local GLV_ATTR_VAL_OFF    = 0x34
local GLV_ATTR_INIT_OFF   = 0x38


local glv_kernel32 = nil
do
    ...
    pcall(function() glv_kernel32 = ffi.load("kernel32") end)
end
if not glv_kernel32 then
    print("[手套] ✗ kernel32 加载失败，模块未启用")
    return
end

local fnGameAlloc, fnGameFree
do
    local hTier0 = glv_kernel32.GetModuleHandleA("tier0.dll")
    if hTier0 == nil then print("[手套] ✗ tier0.dll handle 失败"); return end
    local pAlloc = glv_kernel32.GetProcAddress(hTier0, "MemAlloc_AllocFunc")
    local pFree  = glv_kernel32.GetProcAddress(hTier0, "MemAlloc_FreeFunc")
    if pAlloc == nil or pFree == nil then print("[手套] ✗ MemAlloc 导出失败"); return end
    fnGameAlloc = ffi.cast("void*(__fastcall*)(size_t)", pAlloc)
    fnGameFree  = ffi.cast("void(__fastcall*)(void*)", pFree)
end


local fnSetBodyGroup = nil
do
    local SIG_SBG = "E8 ?? ?? ?? ?? EB 0C 48 8B CF"
    local addr_call = mem.FindPattern("client.dll", SIG_SBG)
    if addr_call then
        local ok, rel32 = pcall(function() return ffi.cast("int32_t*", addr_call + 1)[0] end)
        if ok and rel32 then
            local target = tonumber(addr_call) + 5 + tonumber(rel32)
            fnSetBodyGroup = ffi.cast("void(__fastcall*)(void*, const char*, unsigned int)", target)
        end
    end
end
local glv_sbg_buf = ffi.new("char[?]", 32)
ffi.copy(glv_sbg_buf, "first_or_third_person")


local function glv_is_heap(n) return n and n >= 0x100000000 and n <= 0x7FFFFFFFFFFF end
local function glv_safe_read(addr, ctype)
    local ok, v = pcall(function() return ffi.cast(ctype.."*", addr)[0] end)
    return ok and v or nil
end


local GLOVE_DB = {
    [10085] = {def=4725, label="狂牙手套 翡翠"},
    [10086] = {def=4725, label="狂牙手套 黄色斑纹"},
    [10087] = {def=4725, label="狂牙手套 针尖"},
    [10088] = {def=4725, label="狂牙手套 精神错乱"},
    [10006] = {def=5027, label="血猎手套 焦炭"},
    [10007] = {def=5027, label="血猎手套 蛇咬"},
    [10008] = {def=5027, label="血猎手套 染铜"},
    [10039] = {def=5027, label="血猎手套 游击队"},
    [10018] = {def=5030, label="运动手套 超导体"},
    [10019] = {def=5030, label="运动手套 干旱"},
    [10037] = {def=5030, label="运动手套 潘多拉之盒"},
    [10038] = {def=5030, label="运动手套 树篱迷宫"},
    [10045] = {def=5030, label="运动手套 双栖"},
    [10046] = {def=5030, label="运动手套 青铜形态"},
    [10047] = {def=5030, label="运动手套 欧米伽"},
    [10048] = {def=5030, label="运动手套 迈阿密风云"},
    [10073] = {def=5030, label="运动手套 弹弓"},
    [10074] = {def=5030, label="运动手套 大型猎物"},
    [10075] = {def=5030, label="运动手套 猩红头巾"},
    [10076] = {def=5030, label="运动手套 夜行衣"},
    [10013] = {def=5031, label="驾驶手套 月色织物"},
    [10015] = {def=5031, label="驾驶手套 护卫"},
    [10016] = {def=5031, label="驾驶手套 深红织物"},
    [10040] = {def=5031, label="驾驶手套 菱背蛇纹"},
    [10041] = {def=5031, label="驾驶手套 王蛇"},
    [10042] = {def=5031, label="驾驶手套 蓝紫格子"},
    [10043] = {def=5031, label="驾驶手套 超越"},
    [10044] = {def=5031, label="驾驶手套 墨绿色调"},
    [10069] = {def=5031, label="驾驶手套 绯红列赞"},
    [10070] = {def=5031, label="驾驶手套 雪豹"},
    [10071] = {def=5031, label="驾驶手套 美洲豹女王"},
    [10072] = {def=5031, label="驾驶手套 西装革履"},
    [10009] = {def=5032, label="裹手 皮革"},
    [10010] = {def=5032, label="裹手 云杉 DDPAT"},
    [10021] = {def=5032, label="裹手 屠夫"},
    [10036] = {def=5032, label="裹手 恶土"},
    [10053] = {def=5032, label="裹手 钴蓝骷髅"},
    [10054] = {def=5032, label="裹手 套印"},
    [10055] = {def=5032, label="裹手 防水布胶带"},
    [10056] = {def=5032, label="裹手 森林色调"},
    [10081] = {def=5032, label="裹手 沙漠头巾"},
    [10082] = {def=5032, label="裹手 长颈鹿"},
    [10083] = {def=5032, label="裹手 蟒蛇"},
    [10084] = {def=5032, label="裹手 警告"},
    [10024] = {def=5033, label="摩托手套 日蚀"},
    [10026] = {def=5033, label="摩托手套 薄荷"},
    [10027] = {def=5033, label="摩托手套 嘣"},
    [10028] = {def=5033, label="摩托手套 清凉薄荷"},
    [10049] = {def=5033, label="摩托手套 嘭"},
    [10050] = {def=5033, label="摩托手套 玳瑁"},
    [10051] = {def=5033, label="摩托手套 交运"},
    [10052] = {def=5033, label="摩托手套 多边形"},
    [10077] = {def=5033, label="摩托手套 终点线"},
    [10078] = {def=5033, label="摩托手套 小心烟雾弹"},
    [10079] = {def=5033, label="摩托手套 血压"},
    [10080] = {def=5033, label="摩托手套 第三特种兵连"},
    [10030] = {def=5034, label="专业手套 森林 DDPAT"},
    [10033] = {def=5034, label="专业手套 深红和服"},
    [10034] = {def=5034, label="专业手套 翠绿之网"},
    [10035] = {def=5034, label="专业手套 元勋"},
    [10061] = {def=5034, label="专业手套 深红之网"},
    [10062] = {def=5034, label="专业手套 狩鹿"},
    [10063] = {def=5034, label="专业手套 渐变之色"},
    [10064] = {def=5034, label="专业手套 大腕"},
    [10065] = {def=5034, label="专业手套 渐变大理石"},
    [10066] = {def=5034, label="专业手套 陆军少尉长官"},
    [10067] = {def=5034, label="专业手套 老虎精英"},
    [10068] = {def=5034, label="专业手套 一线特工"},
    [10057] = {def=5035, label="九头蛇手套 翡翠色调"},
    [10058] = {def=5035, label="九头蛇手套 红树林"},
    [10059] = {def=5035, label="九头蛇手套 响尾蛇"},
    [10060] = {def=5035, label="九头蛇手套 表面淬火"},




    [1405] = {def=5030, label="运动手套 紫罗兰珠绣"},
    [1406] = {def=5030, label="运动手套 寒霜"},
    [1407] = {def=5030, label="运动手套 炽烈之炎"},
    [1408] = {def=5030, label="运动手套 奶油细条纹"},
    [1409] = {def=5030, label="运动手套 赤色追风"},
    [1410] = {def=5030, label="运动手套 紫外狂潮"},
    [1417] = {def=5030, label="运动手套 隐知"},

    [1398] = {def=5031, label="驾驶手套 逐沧浪"},
    [1399] = {def=5031, label="驾驶手套 深红鹤服"},
    [1400] = {def=5031, label="驾驶手套 繁花似锦"},
    [1401] = {def=5031, label="驾驶手套 龙拳"},
    [1402] = {def=5031, label="驾驶手套 花园"},
    [1404] = {def=5031, label="驾驶手套 青海波"},
    [1412] = {def=5031, label="驾驶手套 蓝紫点子"},
    [1439] = {def=5031, label="驾驶手套 针织手套"},

    [1413] = {def=5034, label="专业手套 青柠迷彩"},
    [1414] = {def=5034, label="专业手套 黑皮书"},
    [1415] = {def=5034, label="专业手套 可可伯爵"},
    [1416] = {def=5034, label="专业手套 日芒"},
    [1437] = {def=5034, label="专业手套 狂澜"},
    [1438] = {def=5034, label="专业手套 羽袭"},
    [1440] = {def=5034, label="专业手套 青云狩"},
}


local function glv_attr_remove(item_view)
    local vec = item_view + OFF_GLV_ATTR_LIST + OFF_GLV_ATTRIBUTES
    local sz = glv_safe_read(vec, "uint64_t")
    local pt = glv_safe_read(vec + 8, "uint64_t")
    if not sz or tonumber(sz) == 0 then return end
    if pt and glv_is_heap(tonumber(pt)) then
        pcall(function() ffi.cast("uint64_t*", vec)[0] = 0 end)
        pcall(function() ffi.cast("uint64_t*", vec + 8)[0] = 0 end)
        pcall(function() fnGameFree(ffi.cast("void*", tonumber(pt))) end)
    else
        pcall(function() ffi.cast("uint64_t*", vec)[0] = 0 end)
        pcall(function() ffi.cast("uint64_t*", vec + 8)[0] = 0 end)
    end
...
local function glv_attr_create(item_view, paint_kit_id, wear, seed)
    local vec = item_view + OFF_GLV_ATTR_LIST + OFF_GLV_ATTRIBUTES
    local sz = tonumber(glv_safe_read(vec, "uint64_t") or 0)
    local pt = tonumber(glv_safe_read(vec + 8, "uint64_t") or 0)
    if sz ~= 0 or pt ~= 0 then return true end

    local raw = fnGameAlloc(3 * GLV_ATTR_SIZE)
    if raw == nil then return false end
    local raw_addr = tonumber(ffi.cast("uintptr_t", raw))
    if not glv_is_heap(raw_addr) then return false end

    ffi.fill(raw, 3 * GLV_ATTR_SIZE, 0)
    local function w16(off, v) pcall(function() ffi.cast("uint16_t*", raw_addr + off)[0] = v end) end
    local function wf(off, v)  pcall(function() ffi.cast("float*",    raw_addr + off)[0] = v end) end

    w16(0 * GLV_ATTR_SIZE + GLV_ATTR_DEF_OFF,  GLV_ATTR_PAINT)
    wf (0 * GLV_ATTR_SIZE + GLV_ATTR_VAL_OFF,  paint_kit_id)
    wf (0 * GLV_ATTR_SIZE + GLV_ATTR_INIT_OFF, paint_kit_id)
    w16(1 * GLV_ATTR_SIZE + GLV_ATTR_DEF_OFF,  GLV_ATTR_PATTERN)
    wf (1 * GLV_ATTR_SIZE + GLV_ATTR_VAL_OFF,  seed)
    wf (1 * GLV_ATTR_SIZE + GLV_ATTR_INIT_OFF, seed)
    w16(2 * GLV_ATTR_SIZE + GLV_ATTR_DEF_OFF,  GLV_ATTR_WEAR)
    wf (2 * GLV_ATTR_SIZE + GLV_ATTR_VAL_OFF,  wear)
    wf (2 * GLV_ATTR_SIZE + GLV_ATTR_INIT_OFF, wear)

    pcall(function() ffi.cast("uint64_t*", vec)[0] = 3 end)
    pcall(function() ffi.cast("uint64_t*", vec + 8)[0] = ffi.cast("uint64_t", raw_addr) end)
    return true
end


local GLOVE_CONFIG_FILE = "glove_config.txt"
local glove_saved = nil

local function glove_save_config(paint, seed, wear)
    glove_saved = {paint=paint, seed=seed, wear=wear}
    local line = string.format("paint=%d,seed=%d,wear=%.4f", paint, seed, wear)
    pcall(file.Write, GLOVE_CONFIG_FILE, line)
    local lbl = (GLOVE_DB[paint] and GLOVE_DB[paint].label) or ("paint=" .. paint)
    print(string.format("[手套] 配置已保存（%s）", lbl))
end

local function glove_load_config()
    local ok, content = pcall(file.Read, GLOVE_CONFIG_FILE)
    if not ok or not content or content == "" then return end
    local p = content:match("paint=(%d+)")
    local s = content:match("seed=(%d+)")
    local w = content:match("wear=([%d%.]+)")
    if p then
        glove_saved = {
            paint = tonumber(p),
            seed = tonumber(s) or 0,
            wear = tonumber(w) or 0.001,
        }
        local lbl = (GLOVE_DB[glove_saved.paint] and GLOVE_DB[glove_saved.paint].label) or "?"
        print(string.format("[手套] 已加载配置：%s", lbl))
    end
end

local function glove_clear_config()
    glove_saved = nil
    pcall(file.Write, GLOVE_CONFIG_FILE, "")
    print("[手套] 配置已清空")
end
...
local glove_apply_frames    = 0
local glove_last_applied_paint = -1

local function glove_trigger_apply()
    glove_apply_frames = 2
end

local function glove_apply_frame()
    if not glove_saved then return end
    local entry = GLOVE_DB[glove_saved.paint]
    if not entry then return end

    local pawn = entities.GetLocalPawn()
    if not pawn then return end
    local ok_a, alive = pcall(function() return pawn:IsAlive() end)
    if not ok_a or not alive then return end
    local pawn_ptr = GetEntityInstance(pawn:GetIndex())
    if not pawn_ptr then return end

    local item_view = pawn_ptr + OFF_GLV_M_ECON_GLOVES
    local cur_def   = tonumber(glv_safe_read(item_view + OFF_GLV_DEF_IDX, "uint16_t") or 0)
    local cur_acc   = tonumber(glv_safe_read(item_view + 0x1D8, "uint32_t") or 0)
    local vec       = item_view + OFF_GLV_ATTR_LIST + OFF_GLV_ATTRIBUTES
    local attr_sz   = tonumber(glv_safe_read(vec, "uint64_t") or 0)
    local attr_pt   = tonumber(glv_safe_read(vec + 8, "uint64_t") or 0)


    local cur_paint = nil
    if attr_sz > 0 and attr_pt ~= 0 then
        local val = glv_safe_read(attr_pt + GLV_ATTR_VAL_OFF, "float")
        if val then cur_paint = math.floor(val + 0.5) end
    end


    if cur_def ~= entry.def
       or cur_acc == 0
       or cur_paint ~= glove_saved.paint then
        glove_trigger_apply()
    end


    if glove_apply_frames <= 0 then return end
    glove_apply_frames = glove_apply_frames - 1


    pcall(function()
        ffi.cast("uint16_t*", item_view + OFF_GLV_DEF_IDX)[0]        = entry.def
        ffi.cast("int32_t*",  item_view + OFF_GLV_ENTITY_QUALITY)[0] = GLV_QUALITY_UNUSUAL
        ffi.cast("uint32_t*", item_view + 0x1D0)[0] = 0x7FFFFFFF
        ffi.cast("uint32_t*", item_view + 0x1D4)[0] = 0x7FFFFFFF
        ffi.cast("uint32_t*", item_view + 0x1D8)[0] = 12345
        ffi.cast("uint8_t*",  item_view + 0x1E9)[0] = 0
    end)


    if attr_sz == 0 or cur_paint ~= glove_saved.paint then
        if attr_sz > 0 then glv_attr_remove(item_view) end
        glv_attr_create(item_view, glove_saved.paint, glove_saved.wear, glove_saved.seed)
    end


    pcall(function() ffi.cast("uint8_t*", item_view + OFF_GLV_INITIALIZED)[0] = 1 end)


    if fnSetBodyGroup then
        pcall(fnSetBodyGroup, ffi.cast("void*", pawn_ptr), glv_sbg_buf, 1)
    end


    pcall(function() ffi.cast("uint8_t*", pawn_ptr + OFF_GLV_M_BNEED_REAPPLY)[0] = 1 end)
end


local function glove_on_target_change()
    if not glove_saved then return end
    if glove_saved.paint == glove_last_applied_paint then return end
    glove_last_applied_paint = glove_saved.paint
    local pawn = entities.GetLocalPawn()
    if not pawn then return end
    local pawn_ptr = GetEntityInstance(pawn:GetIndex())
    if not pawn_ptr then return end
    local item_view = pawn_ptr + OFF_GLV_M_ECON_GLOVES
    glv_attr_remove(item_view)
    glove_trigger_apply()
end



local GLOVE_CATEGORIES = {
    {def=4725, name="狂牙手套"},
    {def=5027, name="血猎手套"},
    {def=5030, name="运动手套"},
    {def=5031, name="驾驶手套"},
...
local GLOVE_BY_DEF = {}
for pid, e in pairs(GLOVE_DB) do
    GLOVE_BY_DEF[e.def] = GLOVE_BY_DEF[e.def] or {}
    table.insert(GLOVE_BY_DEF[e.def], {pid=pid, label=e.label})
end
for _, list in pairs(GLOVE_BY_DEF) do
    table.sort(list, function(a, b) return a.pid < b.pid end)
end

local glv_cat_names = {}
for _, c in ipairs(GLOVE_CATEGORIES) do table.insert(glv_cat_names, c.name) end


local glv_win = _G._GLOVE_BOX
if not glv_win then

    glv_win = gui.Window("glove_skin", "手套换肤", 555, 100, 380, 430)
    print("[手套] ⚠ 主窗口未提供 glove_box, 已退化为独立窗口")
end

local glv_cat_combo  = gui.Combobox(glv_win, "glv_cat",  "手套类别", unpack(glv_cat_names))
local glv_skin_combo = gui.Combobox(glv_win, "glv_skin", "皮肤",     "(选类别)")

local glv_seed_slider = gui.Slider(glv_win, "glv_seed", "种子",     0, 0, 1000, 1)
local glv_wear_slider = gui.Slider(glv_win, "glv_wear", "磨损",     0, 0, 100,  1)

gui.Button(glv_win, "应用并保存（自动持续应用）", function() _G._GLOVE_DO_SAVE  = true end)
gui.Button(glv_win, "清除配置 (停止自动应用)",   function() _G._GLOVE_DO_CLEAR = true end)
gui.Button(glv_win, "读当前状态",               function() _G._GLOVE_DO_READ  = true end)

local glv_last_cat = -1
local function glove_get_skin_labels(def)
    local list = GLOVE_BY_DEF[def]
    if not list then return {"(空)"} end
    local r = {}
    for _, e in ipairs(list) do table.insert(r, string.format("[%d] %s", e.pid, e.label)) end
    return r
end

callbacks.Register("Draw", "glove_skin_draw", function()



    local ci = glv_cat_combo:GetValue()
    if ci ~= glv_last_cat then
        glv_last_cat = ci
        local cat = GLOVE_CATEGORIES[ci + 1]
        if cat then
            glv_skin_combo:SetOptions(unpack(glove_get_skin_labels(cat.def)))
            glv_skin_combo:SetValue(0)
        end
    end


    if _G._GLOVE_DO_SAVE then
        _G._GLOVE_DO_SAVE = false
        local cat = GLOVE_CATEGORIES[glv_cat_combo:GetValue() + 1]
        if cat then
            local list = GLOVE_BY_DEF[cat.def]
            local item = list and list[glv_skin_combo:GetValue() + 1]
            if item then
                local seed = glv_seed_slider:GetValue() or 0
                local wear = (glv_wear_slider:GetValue() or 0) / 100.0

                if wear < 0.06 then wear = 0.06 end
                glove_save_config(item.pid, seed, wear)
                glove_last_applied_paint = -1
                glove_trigger_apply()
            end
        end
    end

    if _G._GLOVE_DO_CLEAR then
        _G._GLOVE_DO_CLEAR = false
        local pawn = entities.GetLocalPawn()
        if pawn then
            local pawn_ptr = GetEntityInstance(pawn:GetIndex())
            if pawn_ptr then
                local item_view = pawn_ptr + OFF_GLV_M_ECON_GLOVES
                glv_attr_remove(item_view)
                pcall(function() ffi.cast("uint16_t*", item_view + OFF_GLV_DEF_IDX)[0]        = 0 end)
                pcall(function() ffi.cast("int32_t*",  item_view + OFF_GLV_ENTITY_QUALITY)[0] = 0 end)
                pcall(function() ffi.cast("uint8_t*",  item_view + OFF_GLV_INITIALIZED)[0]    = 0 end)
                pcall(function() ffi.cast("uint8_t*",  pawn_ptr + OFF_GLV_M_BNEED_REAPPLY)[0] = 1 end)
            end
        end
        glove_clear_config()
    ...
    if _G._GLOVE_DO_READ then
        _G._GLOVE_DO_READ = false
        local pawn = entities.GetLocalPawn()
        if not pawn then print("[手套] 无 pawn"); return end
        local pawn_ptr = GetEntityInstance(pawn:GetIndex())
        if not pawn_ptr then return end
        local item_view = pawn_ptr + OFF_GLV_M_ECON_GLOVES
        local d = tonumber(glv_safe_read(item_view + OFF_GLV_DEF_IDX, "uint16_t") or 0)
        local i = tonumber(glv_safe_read(item_view + OFF_GLV_INITIALIZED, "uint8_t") or 0)
        local q = tonumber(glv_safe_read(item_view + OFF_GLV_ENTITY_QUALITY, "int32_t") or 0)
        local vec = item_view + OFF_GLV_ATTR_LIST + OFF_GLV_ATTRIBUTES
        local s = tonumber(glv_safe_read(vec, "uint64_t") or 0)
        print(string.format("[手套] def=%d init=%d quality=%d attr_size=%d  saved=%s",
            d, i, q, s, glove_saved and tostring(glove_saved.paint) or "nil"))
    end


    glove_on_target_change()
    glove_apply_frame()
end)



callbacks.Register("FireGameEvent", "glove_skin_evt", function(ev)
    if not glove_saved then return end
    local ok, name = pcall(function() return ev:GetName() end)
    if not ok or not name then return end
    if name == "player_spawn" or name == "round_start"
       or name == "round_prestart" or name == "player_team" then
        glove_trigger_apply()
    end
end)

print("[手套] 模块已加载，72 个皮肤可选；选择 → 应用并保存 → 自动持续应用")
...
end, function(e) print("[错误] " .. tostring(e)) end)
