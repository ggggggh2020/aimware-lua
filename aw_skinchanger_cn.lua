...
local win = gui.Window("universal_skin", "通用皮肤应用 (VirtualProtect)", 100, 100, 450, 440)

gui.Text(win, "===== 皮肤设置 =====")
local weapon_combo = gui.Combobox(win, "wpn_sel", "武器:", unpack(wpn_display_names))
...
local skin_combo = gui.Combobox(win, "skin_sel", "皮肤:", unpack(get_skin_names(weapon_list[1])))

gui.Text(win, "磨损 (Wear 0.00-1.00):")
local wear_slider = gui.Slider(win, "wear_val", "", 0, 0, 100, 1)
local wear_edit = gui.Editbox(win, "wear_edit", "输入精确磨损")
...
gui.Text(win, "种子 (Seed):")
local seed_slider = gui.Slider(win, "seed_val", "", 1, 0, 1000, 1)
gui.Button(win, "应用皮肤", function() _G.APPLY_SKIN = true end)
